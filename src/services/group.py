...
from ..data.db_interactions import create_group, get_group_by_id, update_group
from ..data.schema import GroupData
...
async def get_group(
    group_id: str = Path(..., title="ID of group"), db: Session = Depends(get_db)
):
...
@router.post("/group")
async def start_group(group_data: GroupData, db: Session = Depends(get_db)):
